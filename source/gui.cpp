...
void HookImGui(HDC hdc)
{
	...
	if (GetAsyncKeyState(VK_INSERT) & 1)//菜单开关
	{
		bMenu = !bMenu;
		bool bMenu = true;
	}
	...
	ImColor AllGo_Color = ImColor(0, 255, 0);


	if (AllGo)
	{
		sprintf(AllGo_buf, u8"[F1]  连跳 MCJ 开启");
	}
	else
	{
		AllGo_Color = ImColor(255, 0, 0);
		sprintf(AllGo_buf, u8"[F1]  连跳 MCJ 关闭");
	}


	ImGui::GetOverlayDrawList()->AddText({ 10.f,40.f }, AllGo_Color, AllGo_buf);
