...
void AdjustSpeed(double speed) {
	...
	DWORD old_prot;
	VirtualProtect((void*)c_Offset.dwSpeedPointer, sizeof(double), PAGE_READWRITE, &old_prot);
	if (speed != LastSpeed) {
		*reinterpret_cast<double*>(c_Offset.dwSpeedPointer) = (speed * 1000);
		LastSpeed = speed;
	}
	VirtualProtect((void*)c_Offset.dwSpeedPointer, sizeof(double), old_prot, &old_prot);
}
